...
ExecInitHashJoin(HashJoin *node, EState *estate, int eflags)
{
	 ...
	 */
	{
	 ...
	 /*
		CSI Joseph: The following block doesn't appear here.
		It used to be where the following "initialize child expressions" code was,
		though another block of code with the header "initialize child expressions"
		appears earlier in the original project file (ctrl-f it).
		Basically, I'm not sure we have to use it.
...
ExecHashJoinReInitializeDSM(HashJoinState *state, ParallelContext *pcxt)
{
	// Updated - Josh
	HashJoinTable hashtable;
	if (state->hj_InnerProbing) {
		hashtable = state->hj_InnerHashTable;
	}
	else {
		hashtable = state->hj_OuterHashTable;
	}
